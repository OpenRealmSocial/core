...
import { getOpenrealmTime } from "./time";
import * as validations from "./validations";
import { PublicClients, defaultPublicClients } from "./eth/clients";

/** Internal Types  */

type MessageDataOptions = Pick<protobufs.MessageData, "qid" | "network"> & {
  timestamp?: number; // Openrealm timestamp
};
...
const makeMessageData = async <TData extends protobufs.MessageData>(
  ...
  if (!dataOptions.timestamp) {
    getOpenrealmTime().map((timestamp) => {
