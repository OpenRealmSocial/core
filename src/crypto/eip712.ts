...
export const EIP_712_OPENREALM_DOMAIN = {
  name: "Openrealm Verify Ethereum Address",
  version: "2.0.0",
  // fixed salt to minimize collisions
  salt: "0xf2d857f4a3edcb9b78b4d503bfe733db1e3f6cdc2b7971ee739626c97e86a558",
} as const;

export const EIP_712_OPENREALM_VERIFICATION_CLAIM = [
  {
    name: "qid",
    type: "uint256",
  },
...
export const EIP_712_OPENREALM_VERIFICATION_CLAIM_CHAIN_IDS = [...CHAIN_IDS, 0];

export const EIP_712_OPENREALM_MESSAGE_DATA = [
  {
    name: "hash",
    type: "bytes",
  },
...
export const EIP_712_USERNAME_DOMAIN = {
  ...
  version: "1",
  chainId: 1,
  verifyingContract: "0xe3be01d99baa8db9905b33a3ca391238234b79d1", // name registry contract, will be the openrealm ENS CCIP contract later
} as const;
...
export const verifyVerificationClaimEOASignature = async (
  ...
  const valid = await ResultAsync.fromPromise(
    verifyTypedData({
      ...
      domain: EIP_712_OPENREALM_DOMAIN,
      types: { VerificationClaim: EIP_712_OPENREALM_VERIFICATION_CLAIM },
...
export const verifyVerificationClaimContractSignature = async (
  ...
  const valid = await ResultAsync.fromPromise(
    client.verifyTypedData({
      address: bytesToHex(address),
      domain: { ...EIP_712_OPENREALM_DOMAIN, chainId },
      types: { VerificationClaim: EIP_712_OPENREALM_VERIFICATION_CLAIM },
      primaryType: "VerificationClaim",
      message: claim,
      signature,
...
export const verifyVerificationEthAddressClaimSignature = async (
  ...
  if (!EIP_712_OPENREALM_VERIFICATION_CLAIM_CHAIN_IDS.includes(chainId)) {
    return ResultAsync.fromPromise(
      Promise.reject(),
      () => new HubError("bad_request.invalid_param", "Invalid chain ID"),
    );
...
export const verifyMessageHashSignature = async (
  ...
  const valid = await ResultAsync.fromPromise(
    verifyTypedData({
      address: bytesToHex(address),
      domain: EIP_712_OPENREALM_DOMAIN,
      types: { MessageData: EIP_712_OPENREALM_MESSAGE_DATA },
