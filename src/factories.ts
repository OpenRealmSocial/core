...
import { OPENREALM_EPOCH, getOpenrealmTime, toOpenrealmTime } from "./time";
import { VerificationEthAddressClaim } from "./verifications";
import { LocalAccount } from "viem";

/** Scalars */

const QidFactory = Factory.define<number>(() => {
...
const QnameFactory = Factory.define<Uint8Array>(() => {
...
const CastIdFactory = Factory.define<protobufs.CastId>(() => {
  return protobufs.CastId.create({
    qid: QidFactory.build(),
    hash: MessageHashFactory.build(),
  });
});

const OpenrealmNetworkFactory = Factory.define<protobufs.OpenrealmNetwork>(() => {
  return faker.helpers.arrayElement([
    protobufs.OpenrealmNetwork.DEVNET,
    protobufs.OpenrealmNetwork.MAINNET,
    protobufs.OpenrealmNetwork.TESTNET,
  ]);
});
...
const MessageDataFactory = Factory.define<protobufs.MessageData>(() => {
  return protobufs.MessageData.create({
    qid: QidFactory.build(),
    timestamp: getOpenrealmTime()._unsafeUnwrap(),
    network: protobufs.OpenrealmNetwork.TESTNET,
  });
});
...
const CastAddBodyFactory = Factory.define<protobufs.CastAddBody>(() => {
  ...
  return protobufs.CastAddBody.create({
    ...
    mentions: [QidFactory.build(), QidFactory.build(), QidFactory.build()],
...
const VerificationEthAddressClaimFactory = Factory.define<VerificationEthAddressClaim>(() => {
  ...
  return {
    qid: BigInt(QidFactory.build()),
    address,
    network: OpenrealmNetworkFactory.build(),
    blockHash,
  };
});
...
>(({ onCreate, transientParams }) => {
  onCreate(async (body) => {
    ...
    if (body.ethSignature.length === 0) {
      ...
      const qid = transientParams.qid ?? QidFactory.build();
      const network = transientParams.network ?? OpenrealmNetworkFactory.build();
  ...
  onCreate(async (data) => {
    ...
    if (body.ethSignature.length === 0) {
      const signedBody = await VerificationAddEthAddressBodyFactory.create(body, {
        transient: { qid: data.qid, network: data.network, signer: transientParams.signer },
      });
...
const UsernameProofDataFactory = Factory.define<protobufs.UsernameProofData>(() => {
  ...
  return MessageDataFactory.build({
    ...
    timestamp: toOpenrealmTime(proofBody.timestamp * 1000)._unsafeUnwrap(),
    qid: proofBody.qid,
...
const OnChainEventFactory = Factory.define<protobufs.OnChainEvent>(() => {
  return protobufs.OnChainEvent.create({
    ...
    blockHash: BlockHashFactory.build(),
    blockTimestamp: Math.floor(faker.datatype.datetime({ min: OPENREALM_EPOCH, max: Date.now() }).getTime() / 1000),
...
const UserNameProofFactory = Factory.define<protobufs.UserNameProof>(() => {
  return protobufs.UserNameProof.create({
    ...
    name: QnameFactory.build(),
    qid: QidFactory.build(),
    type: protobufs.UserNameType.USERNAME_TYPE_QNAME,
  });
});
...
const StorageRentEventBodyFactory = Factory.define<protobufs.StorageRentEventBody>(() => {
  return StorageRentEventBody.create({
    ...
    expiry: getOpenrealmTime()._unsafeUnwrap() + 60 * 60 * 24 * 365, // a year
  });
});
...
export const Factories = {
  ...
  OpenrealmNetwork: OpenrealmNetworkFactory,
