...
function createBaseHubState(): HubState {
  return { lastQnameProof: 0, lastL2Block: 0 };
}

export const HubState = {
  encode(message: HubState, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.lastQnameProof !== 0) {
      writer.uint32(16).uint64(message.lastQnameProof);
    }
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 2:
          ...
          message.lastQnameProof = longToNumber(reader.uint64() as Long);
          continue;
        case 3:
          if (tag != 24) {
            break;
  ...
  fromJSON(object: any): HubState {
    return {
      lastQnameProof: isSet(object.lastQnameProof) ? Number(object.lastQnameProof) : 0,
  ...
  toJSON(message: HubState): unknown {
    const obj: any = {};
    message.lastQnameProof !== undefined && (obj.lastQnameProof = Math.round(message.lastQnameProof));
  ...
  fromPartial<I extends Exact<DeepPartial<HubState>, I>>(object: I): HubState {
    const message = createBaseHubState();
    message.lastQnameProof = object.lastQnameProof ?? 0;
