...
import {
  EIP_712_OPENREALM_DOMAIN,
  EIP_712_OPENREALM_MESSAGE_DATA,
  EIP_712_OPENREALM_VERIFICATION_CLAIM,
...
export class EthersEip712Signer extends Eip712Signer {
  ...
  public async signMessageHash(hash: Uint8Array): HubAsyncResult<Uint8Array> {
    const hexSignature = await ResultAsync.fromPromise(
      this._ethersSigner.signTypedData(
        EIP_712_OPENREALM_DOMAIN,
        { MessageData: [...EIP_712_OPENREALM_MESSAGE_DATA] },
        { hash },
      ),
  ...
  public async signVerificationEthAddressClaim(
    ...
    const domain = chainId === 0 ? EIP_712_OPENREALM_DOMAIN : { ...EIP_712_OPENREALM_DOMAIN, chainId };
    const hexSignature = await ResultAsync.fromPromise(
      this._ethersSigner.signTypedData(domain, { VerificationClaim: [...EIP_712_OPENREALM_VERIFICATION_CLAIM] }, claim),
      (e) => new HubError("bad_request.invalid_param", e as Error),
    );
