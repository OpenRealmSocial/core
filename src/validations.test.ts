...
import { fromOpenrealmTime, getOpenrealmTime } from "./time";
import * as validations from "./validations";
import { makeVerificationEthAddressClaim } from "./verifications";
import { UserDataType, UserNameType } from "@openrealm/hub-nodejs";
...
describe("validateQid", () => {
  test("succeeds", () => {
    ...
    expect(validations.validateQid(qid)).toEqual(ok(qid));
  });

  test("fails with 0", () => {
    expect(validations.validateQid(0)).toEqual(err(new HubError("bad_request.validation_failure", "qid is missing")));
  });

  test("fails with negative number", () => {
    expect(validations.validateQid(-1)).toEqual(
      err(new HubError("bad_request.validation_failure", "qid must be positive")),
    );
  });

  test("fails when undefined", () => {
    expect(validations.validateQid(undefined)).toEqual(
      err(new HubError("bad_request.validation_failure", "qid is missing")),
    );
  });

  test("fails with non-integer number", () => {
    expect(validations.validateQid(1.5)).toEqual(
      err(new HubError("bad_request.validation_failure", "qid must be an integer")),
    );
  });
});

describe("validateQname", () => {
  test("succeeds with valid byte array input", () => {
    const qname = Factories.Qname.build();
    expect(validations.validateQname(qname)).toEqual(ok(qname));
  });

  test("succeeds with valid string inpt", () => {
    const qname = bytesToUtf8String(Factories.Qname.build())._unsafeUnwrap();
    expect(validations.validateQname(qname)).toEqual(ok(qname));
  });

  test("fails when greater than 16 characters", () => {
    const qname = faker.random.alpha(17);
    expect(validations.validateQname(qname)).toEqual(
      err(new HubError("bad_request.validation_failure", `qname "${qname}" > 16 characters`)),
    );
  });
  ...
  test("fails with an empty string", () => {
    const qname = "";
    expect(validations.validateQname(qname)).toEqual(
      err(new HubError("bad_request.validation_failure", "qname is missing")),
    );
  });

  test("fails when undefined", () => {
    expect(validations.validateQname(undefined)).toEqual(
      err(new HubError("bad_request.validation_failure", "qname is missing")),
    );
  });
  ...
  test("fails with invalid characters", () => {
    const qname = "@qname";
    expect(validations.validateQname(qname)).toEqual(
      err(new HubError("bad_request.validation_failure", `qname "${qname}" doesn't match ${validations.QNAME_REGEX}`)),
    );
  });

  test("does not allow names ending with .eth", () => {
    const qname = "qname.eth";
    expect(validations.validateQname(qname)).toEqual(
      err(new HubError("bad_request.validation_failure", `qname "${qname}" doesn't match ${validations.QNAME_REGEX}`)),
    );
  });
...
describe("validateENSname", () => {
  ...
  test("fails with invalid characters", () => {
    const ensName = "-qname.eth";
    expect(validations.validateEnsName(ensName)).toEqual(
      err(
        new HubError("bad_request.validation_failure", `ensName "${ensName}" doesn't match ${validations.QNAME_REGEX}`),
      ),
    );
  });
...
describe("validateCastId", () => {
  ...
  test("fails when qid is invalid", async () => {
    const castId = Factories.CastId.build({ qid: 0 });
  ...
  test("fails when both qid and tsHash are invalid", async () => {
    const castId = Factories.CastId.build({ qid: undefined, hash: undefined });
    expect(validations.validateCastId(castId)).toEqual(
      err(new HubError("bad_request.validation_failure", "qid is missing, hash is missing")),
    );
  });
});
...
describe("validateCastAddBody", () => {
  ...
  describe("fails", () => {
    ...
    test("with an invalid embed CastId", () => {
      body = Factories.CastAddBody.build({
        embeds: [{ castId: Factories.CastId.build({ qid: undefined }) }],
      });
      hubErrorMessage = "qid is missing";
    });

    test("when parent qid is missing", () => {
      body = Factories.CastAddBody.build({
        parentCastId: Factories.CastId.build({ qid: undefined }),
      });
      hubErrorMessage = "qid is missing";
    });
    ...
    test("with non-integer mentionsPositions", () => {
      body = Factories.CastAddBody.build({ mentions: [Factories.Qid.build()], mentionsPositions: [1.5] });
...
describe("validateReactionBody", () => {
  ...
  describe("fails", () => {
    ...
    test("when cast qid is missing", () => {
      body = Factories.ReactionBody.build({
        targetCastId: Factories.CastId.build({ qid: undefined }),
      });
      hubErrorMessage = "qid is missing";
    });
...
describe("validateVerificationAddEthAddressBody", () => {
  ...
  test("succeeds", async () => {
    const body = await Factories.VerificationAddEthAddressBody.create({}, { transient: { qid, network } });
    const result = await validations.validateVerificationAddEthAddressBody(body, qid, network);
...
describe("validateVerificationAddEthAddressSignature", () => {
  ...
  test("succeeds for eoas", async () => {
    const body = await Factories.VerificationAddEthAddressBody.create({}, { transient: { qid, network } });
    const result = await validations.validateVerificationAddEthAddressSignature(body, qid, network);
  ...
  test("succeeds for contract signatures", async () => {
    ...
    const body = await Factories.VerificationAddEthAddressBody.create(
      ...
      { transient: { qid, network, contractSignature: true } },
    );
    const result = await validations.validateVerificationAddEthAddressSignature(body, qid, network, publicClients);
    expect(result.isOk()).toBeTruthy();
  });
  ...
  test("fails with eth signature from different address", async () => {
    ...
    const claim = makeVerificationEthAddressClaim(qid, ethSignerKey, network, blockHash)._unsafeUnwrap();
...
describe("validateUsernameProof", () => {
  ...
  test("when type is qname", async () => {
    const proof = await Factories.UsernameProofMessage.create({
      data: { usernameProofBody: { type: UserNameType.USERNAME_TYPE_QNAME } },
    });
    const result = await validations.validateUsernameProofBody(proof.data.usernameProofBody, proof.data);
    expect(result._unsafeUnwrapErr()).toEqual(
...
describe("validateMessageData", () => {
  test("fails with timestamp more than 10 mins in the future", async () => {
    const data = Factories.MessageData.build({
      timestamp: getOpenrealmTime()._unsafeUnwrap() + validations.ALLOWED_CLOCK_SKEW_SECONDS + 1,
    });
  ...
  describe("with stubbed Date.now", () => {
    ...
    beforeAll(() => {
      ...
      global.Date.now = () => fromOpenrealmTime(validations.EMBEDS_V1_CUTOFF)._unsafeUnwrap();
    });
