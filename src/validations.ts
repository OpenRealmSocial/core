...
export const QNAME_REGEX = /^[a-z0-9][a-z0-9-]{0,15}$/;
...
export const validateCastId = (castId?: protobufs.CastId): HubResult<protobufs.CastId> => {
  ...
  return Result.combineWithAllErrors([validateQid(castId.qid), validateMessageHash(castId.hash)])
    .map(() => castId)
    .mapErr(
...
export const validateQid = (qid?: number | null): HubResult<number> => {
  if (typeof qid !== "number" || qid === 0) {
    return err(new HubError("bad_request.validation_failure", "qid is missing"));
  }

  if (qid < 0) {
    return err(new HubError("bad_request.validation_failure", "qid must be positive"));
  }

  if (!Number.isInteger(qid)) {
    return err(new HubError("bad_request.validation_failure", "qid must be an integer"));
  }
...
export const validateMessageData = async <T extends protobufs.MessageData>(
  ...
  // 1. Validate qid
  const validQid = validateQid(data.qid);
  if (validQid.isErr()) {
    return err(validQid.error);
  }

  const openrealmTime = getOpenrealmTime();
  if (openrealmTime.isErr()) {
    return err(openrealmTime.error);
  }

  // 2. Validate timestamp
  if (data.timestamp - openrealmTime.value > ALLOWED_CLOCK_SKEW_SECONDS) {
...
export const validateVerificationAddEthAddressSignature = async (
  ...
  qid: number,
  network: protobufs.OpenrealmNetwork,
  publicClients: PublicClients = defaultPublicClients,
): HubAsyncResult<Uint8Array> => {
  ...
  const reconstructedClaim = makeVerificationEthAddressClaim(qid, body.address, network, body.blockHash);
...
export const validateCastAddBody = (
  ...
  for (let i = 0; i < body.mentions.length; i++) {
    const mention = validateQid(body.mentions[i]);
    if (mention.isErr()) {
...
export const validateNetwork = (network: number): HubResult<protobufs.OpenrealmNetwork> => {
  if (!Object.values(protobufs.OpenrealmNetwork).includes(network)) {
...
export const validateVerificationAddEthAddressBody = async (
  ...
  const validSignature = await validateVerificationAddEthAddressSignature(body, qid, network, publicClients);
...
export const validateUsernameProofBody = (
  ...
  if (body.qid !== data.qid) {
    return err(
      new HubError("bad_request.validation_failure", "qid in username proof does not match qid in message data"),
    );
  }

  // Proof time is in Unix seconds
  const proofOpenrealmTimestamp = toOpenrealmTime(body.timestamp * 1000);
  if (proofOpenrealmTimestamp.isErr()) {
    return err(proofOpenrealmTimestamp.error);
  }
  if (proofOpenrealmTimestamp.value !== data.timestamp) {
    return err(
      new HubError(
...
export const validateUserDataAddBody = (body: protobufs.UserDataBody): HubResult<protobufs.UserDataBody> => {
  ...
  switch (type) {
    ...
    case protobufs.UserDataType.USERNAME: {
      // Users are allowed to set qname = '' to remove their qname, otherwise we need a valid qname to add
      if (value !== "") {
        const validatedQname = validateQname(value);
        const validatedEnsName = validateEnsName(value);
        // At least one of qname or ensName must be valid
        if (validatedQname.isErr() && validatedEnsName.isErr()) {
          return err(validatedQname.error);
        }
      }
      break;
    }
...
export const validateQname = <T extends string | Uint8Array>(qnameP?: T | null): HubResult<T> => {
  if (qnameP === undefined || qnameP === null || qnameP === "") {
    return err(new HubError("bad_request.validation_failure", "qname is missing"));
  }

  let qname;
  if (qnameP instanceof Uint8Array) {
    const fromBytes = bytesToUtf8String(qnameP);
    if (fromBytes.isErr()) {
      return err(fromBytes.error);
    }
    qname = fromBytes.value;
  } else {
    qname = qnameP;
  }

  if (qname === undefined || qname === null || qname === "") {
    return err(new HubError("bad_request.validation_failure", "qname is missing"));
  }

  // QNAME_MAX_LENGTH - ".eth".length
  if (qname.length > 16) {
    return err(new HubError("bad_request.validation_failure", `qname "${qname}" > 16 characters`));
  }

  const hasValidChars = QNAME_REGEX.test(qname);
  if (hasValidChars === false) {
    return err(new HubError("bad_request.validation_failure", `qname "${qname}" doesn't match ${QNAME_REGEX}`));
  }
...
export const validateEnsName = <T extends string | Uint8Array>(ensNameP?: T | null): HubResult<T> => {
  ...
  const hasValidChars = QNAME_REGEX.test(nameParts[0]);
  if (!hasValidChars) {
    return err(new HubError("bad_request.validation_failure", `ensName "${ensName}" doesn't match ${QNAME_REGEX}`));
  }
